...
function CardAdminGame({ item }) {
    ...
    const update = () => {
        if (field === "percentage") {
            const body = {
                ...
                priceOffer: (
                    data.price *
                    ((100 - input.current.value) / 100)
                ).toFixed(2),
            };
            updateGame(item._id, { body })
                .then((res) => {
                    reset();
    ...
    return (
        <div className="card-user-admin">
            ...
            {data.offer && (
                <div className="card-user-admin-fields">
                    <p className="text-center">Offer : $ {data.priceOffer} </p>
                </div>
            )}
            <div className="buttons-administation-game">
                <button
                    className="btns-games"
                    onClick={() => handleInput("New price", "price", "number")}
                >
                    Update price
                </button>
                {!data.trailer && (
                    <button
                        className="btns-games"
                        onClick={() =>
                            handleInput("Url trailer", "trailer", "text")
                        }
                    >
                        Add trailer
                    </button>
                ...
                <button
                    className="btns-games"
                    onClick={() =>
                        handleInput("Url screenshot", "screenshot", "text")
                    }
                >
                    Add screenshot
                </button>
            ...
            <div className="buttons-administation-offer">
                {data.offer ? (
                    <>
                        <button
                            className="btns-games o"
                            onClick={() =>
                                handleInput(
                                    "New offer: percentage %",
                                    "percentage",
                                    "number"
                                )
                            }
                        ...
                        <button className="btns-games o" onClick={removeOffer}>
                            Remove offer
                        </button>
                    </>
                ) : (
            ...
            <button className="btns-games" onClick={removeGame}>
                Delete game
            </button>
