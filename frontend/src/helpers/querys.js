...
export const updateGame = async(id,body) => {
    try{
        const res = await axios.put(`http://localhost:4000/api/game/${id}`,{...body})
...
export const addGame = async(body)=>{
    try{
        ...
        const res = await axios.post('http://localhost:4000/api/game',{...body})
...
export const getGameByGenre = async(genre) => {
    try{
        const res = await axios.get('http://localhost:4000/api/gameByGenre/'+genre)
        return res
    }catch(err){console.log(err)}
}
...
export const searchGame = async(text) => {
    try{
        const res = await axios.get('http://localhost:4000/api/gameByName/'+text)
        return res.data
    }catch(err){console.log(err)}
}
